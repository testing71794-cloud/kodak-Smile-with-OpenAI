...
function Build-RunnerCmdLine(
    ...
    $parts = @(
        ...
        (Escape-CmdArg ([string]$ExpectedDeviceCount)),
        (Escape-CmdArg $SyncKey)
    )
    return ($parts -join " ")
}
...
function Run-FlowBatch([string]$RepoRoot,[string]$Suite,[System.IO.FileInfo]$Flow,[string[]]$Devices,[string]$AppId,[string]$ClearState,[string]$MaestroCmdArg,[string]$IncludeTagArg,[string]$Label="Running") {
    ...
    $SyncDir = Join-Path $StatusDir "sync"
    ...
    $expectedCount = $Devices.Count
    $syncKey = "{0}_{1}_{2}" -f $Suite, $flowName.Replace(' ','_'), $Label.ToLower()

    New-Item -ItemType Directory -Force -Path $SyncDir | Out-Null
    Get-ChildItem -LiteralPath $SyncDir -Filter "$syncKey*.ready" -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
    ...
    foreach ($device in $Devices) {
        $cmdLine = Build-RunnerCmdLine $RunnerBat $Suite $flowPath $device $AppId $ClearState $MaestroCmdArg $IncludeTagArg $expectedCount $syncKey
        try {
